...
// 	            },
		...
		// 基本功能菜单加载-------->通过
		$.post("${pageContext.request.contextPath}/auth/functionAction_findFunctionById",function(data){
		...
		$("#btnEp").click(function(){
			//   修改密码实现方法  
			//  1: 获取新密码   判断密码是否有效   js 校验  txtNewPass  新密码   txtRePass 确认密码
			...
			// 空白字符  ...    js  手册
			  var reg = /\s+/; //  出现一个或者多个空白字符 否可以匹配到
			  if(reg.test(newPwd)){
					$.messager.alert('警告','密码不能有空白字符','warning'); 
					  return;
			  }
			  
			  if(newPwd.length<=0||newPwd.length>=7){
					$.messager.alert('警告','密码1-6位','warning'); 
					  return;
			  }
			...
			// 3: 密码一致 有效 发送ajax请求 给action  --->service--->Dao  数据库 当前用户密码的update操作
			
			$.post("${pageContext.request.contextPath}/user/userAction_editPassword",{"password":$("#txtNewPass").val()},function(data){
				  if(data){
				...
				<tr>
					<td style="width: 300px;">
						<div style="color: #999; font-size: 8pt;">
							强强科技 | Powered by <a href="http://www.qqkj.com/">www.baidu.com</a>
						</div>
					</td>
