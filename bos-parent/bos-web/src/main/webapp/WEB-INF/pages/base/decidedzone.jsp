...
<script type="text/javascript">
	...
	function doAssociations(){
	    ...
	    //  左右两边 清空   remove  empty  
	      $("#noassociationSelect").empty();
	      $("#associationSelect").empty();
		// 2: 立刻发送2次 ajax 访问 bos action--->service--->restful --->crm  List<Cusotmer>
		$.post("${pageContext.request.contextPath}/bc/decidedZoneAction_findnoassociationCustomers",function(data){
			//  data List<Customer>  加载左边  noassociationSelect
			  $(data).each(function(){
				  $("#noassociationSelect").append("<option value='"+this.id+"'>"+this.name+"</option>");
			  });
		});
		$.post("${pageContext.request.contextPath}/bc/decidedZoneAction_findassociationCustomers",{"id":arr[0].id},function(data){
			//  data List<Customer>  加载右边  根据选中定区id  访问 crm  获取当前定区已经关联客户信息  associationSelect
			 $(data).each(function(){
	...
	$(function(){
		...
		$('#grid').datagrid( {
			...
			pagination : true,
			toolbar : toolbar,
			url : "${pageContext.request.contextPath}/bc/decidedZoneAction_pageQuery",
			idField : 'id',
			columns : columns,
		...
		$("#btn").click(function(){
			  var jsonObj = $("#searchDecidedZoneForm").serializeJson();
			    // 2"  load 方法 自动调用 datagrid url 地址请求
			      $("#grid").datagrid("load",jsonObj);//  条件+page  rows
			    // 3: 关闭窗口
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="addDecidedZoneForm" action="${pageContext.request.contextPath }/bc/decidedZoneAction_save" method="post">
					...
					<tr>
						...
						<td>
							<input class="easyui-combobox" name="staff.id"  
    							data-options="valueField:'id',textField:'name',url:'${pageContext.request.contextPath }/bc/staffAction_ajaxList'" />  
						</td>
					</tr>
					<tr height="300">
						...
						<td>
							...
							data-options="url:'${pageContext.request.contextPath }/bc/subAreaAction_findnoassociation',fitColumns:true,singleSelect:false">
								<thead>  
							        <tr>  
	...
	<div class="easyui-window" title="关联客户窗口" id="customerWindow" collapsible="false" closed="true" minimizable="false" maximizable="false" style="top:20px;left:200px;width: 400px;height: 300px;">
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="customerForm" action="${pageContext.request.contextPath }/bc/decidedZoneAction_assignCustomersToDecidedZone" method="post">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
